...
const createOfferedCourseInoDB = async (payload: TOfferedCourse) => {
  ...
  if(isSameOfferedCourseExistsWithSameRegisteredSemesterWithSameSection){
    throw new AppError(httpStatus.NOT_FOUND, `Offered course with same section already exists!`)
  }
  ...
  if(hasTimeConflict(assignSchedule, newSchedule)){
    throw new AppError(httpStatus.CONFLICT, `The faculty is not available is that time ! Choose other time or date.`)
  };
...
const getSingleOfferedCourseInoDB = async (
  id: string,
) => {
...
const deleteOfferedCourseInoDB = async (
  id: string,
) => {

  const isOfferedCourseExist = await OfferedCourse.findById(id).populate('semesterRegistration');
  ...
  if(semesterRegistrationStatus !== 'UPCOMING'){
    throw new AppError(httpStatus.BAD_REQUEST, `You can not delete this semester in status : ${semesterRegistrationStatus}`);
  };
...
const updateOfferedCourseInoDB = async (
  ...
  if(semesterRegistrationStatus?.status !== 'UPCOMING'){
    throw new AppError(httpStatus.BAD_REQUEST, `You can't update this offered course as it is ${semesterRegistrationStatus?.status}`)
  }
